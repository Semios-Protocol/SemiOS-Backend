...
@Slf4j
@Service
public class D4aMintChainServiceImpl implements SubscriberChainService {

    //例： https://goerli.etherscan.io/tx/0xb6d1521c5fc85d5aa89821c8007137007ceca1b841a71a27ef95ac1b5881e070#eventlog
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        ...
        log.info("[D4AMintChainServiceImpl] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        log.info("[D4AMintChainServiceImpl] transactionHash:{} amount0:{} amount1:{} pairAddress:{}", transactionDto.getTransactionHash(), amount0, amount1, pairAddress);
        ...
        LiquidityTransaction liquidityTransaction = liquidityTransactionService.selectByTransactionHashAndType(erc20Liquidity.getErc20Address(), transactionDto.getTransactionHash(), LiquidityTradeTypeEnum.ADD.getStatus());
        if (liquidityTransaction != null) {
            log.info("[D4AMintChainServiceImpl] had handle transactionHash:{} pairAddress:{}", transactionDto.getTransactionHash(), pairAddress);
            return;
        }
        ...
        liquidityTransaction.setTradeType(LiquidityTradeTypeEnum.ADD.getStatus());
        ...
        BigDecimal amount0Decimal = new BigDecimal(amount0).divide(CommonUtil.getPowBigDecimal(Integer.valueOf(decimal)), 18, RoundingMode.FLOOR);
        BigDecimal amount1Decimal = new BigDecimal(amount1).divide(CommonUtil.getPowBigDecimal(Integer.valueOf(decimal)), 18, RoundingMode.FLOOR);
        ...
        log.info("[D4AMintChainServiceImpl] ethGetTransactionByHash result:{}", result.getData());
        if (result.getResultCode() != ResultDesc.SUCCESS.getResultCode()) {
            log.error("[D4AMintChainServiceImpl] ethGetTransactionByHash error result:{}", result.getResultDesc());
        ...
        if (objectMap == null) {
            log.error("[D4AMintChainServiceImpl] objectMap is null result:{}", result.getData());
            throw new RuntimeException(transactionDto.getTransactionHash() + "cannot find creator");
        }
        String from = (String) objectMap.get("from");
        log.info("[D4AMintChainServiceImpl] ethGetTransactionByHash creator:{}", from);
