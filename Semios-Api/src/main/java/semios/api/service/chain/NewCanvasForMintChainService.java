...
@Slf4j
@Service
public class NewCanvasForMintChainService implements SubscriberChainService {
...
//        System.out.println(JacksonUtil.obj2json(stringList));
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[NewCanvasForMintChainService]transactionDto:{}", JacksonUtil.obj2json(transactionDto));
        ...
        if (canvas != null) {
            log.error("[NewCanvasForMintChainService] canvas uri:{} is exist transactionDto:{}", uri,
                    JacksonUtil.obj2json(transactionDto));
            return;
        }

        NewCanvasUriDto newCanvasUriDto = restTemplate.getForObject(uri, NewCanvasUriDto.class);
        log.info("[NewCanvasForMintChainService]uri return canvasInfo:{}", JacksonUtil.obj2json(newCanvasUriDto));
        if (newCanvasUriDto == null) {
            log.error("[NewCanvasForMintChainService] canvas uri:{} is null transactionDto:{}", uri,
                    JacksonUtil.obj2json(transactionDto));
        ...
        if (result.getResultCode() != ResultDesc.SUCCESS.getResultCode()) {
            log.error("[NewCanvasForMintChainService] error result:{} transactionDto:{}", result.getResultDesc(),
                    JacksonUtil.obj2json(transactionDto));
            throw new RuntimeException("保存canvas查询价格信息失败");
        }
        log.info("[NewCanvasForMintChainService]infura return data:{}", result.getData());
        ...
        if (indexResult.getResultCode() != ResultDesc.SUCCESS.getResultCode()) {
            log.error("[NewCanvasForMintChainService] index error result:{}", indexResult.getResultDesc());
            throw new RuntimeException("保存canvas查询index信息失败");
        }
        log.info("[NewCanvasForMintChainService]infura index return data:{}", indexResult.getData());
        ...
        if (dao == null) {
            log.error("[NewCanvasForMintChainService] daoDetailByProjectId error dao is null");
        ...
        if (canvas.getOwnerAddress().equals(dao.getOwnerAddress())) {
            ...
            if (dao.getNeedMintableWork() == 0 && DaoStatusEnum.STARTED.getStatus().equals(dao.getDaoStatus())) {
                ...
                log.info("[NewCanvasForMintChainService] addWork daoId:{} workNum:{} result:{}", dao.getId(), 1, addWork);
                if (addWork) {
                    log.info("[NewCanvasForMintChainService] addWork success daoId:{}", dao.getId());
                } else {
                    log.error("[NewCanvasForMintChainService] addWork fail daoId:{}", dao.getId());
