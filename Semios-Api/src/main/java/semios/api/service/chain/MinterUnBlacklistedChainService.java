 ...
 * 消除 minter 黑名单
 *
 * @description: 消除 minter 黑名单
 * @author: xiangbin
 * @create: 2022-08-25 13:43
 **/
@Slf4j
@Service
public class MinterUnBlacklistedChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        ...
        log.info("[MinterUnBlacklistedChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        log.info("[MinterUnBlacklistedChainService] daoId:{} account:{}", daoId, account);

        Dao dao = daoService.daoDetailByProjectId(daoId);
        if (dao == null) {
            throw new RuntimeException("MinterUnBlacklistedChainService cannot find dao");
        }
        ...
        Dao dao1 = new Dao();
        if (daoStrategy == null) {
            throw new RuntimeException("MinterUnBlacklistedChainService cannot find daoStrategy!");
        } else {
            ...
            if (addressSet.size() == 0) {
                ...
                dao1.setMinterWorksBlacklist((DaoBlackListEnum.CLOSE.getStatus()));
                log.info("[MinterUnBlacklistedChainService] dao setMinterWorksBlacklist CLOSE");
            } else {
                dao1 = null;
            }
        }
