...
/**
 * 转移Edit Information权限所有dao均有
 *
 * @description: 转移Edit Information权限所有dao均有
 * @author: zhyyao
 * @create: 2024-07-17 13:43
 **/
@Slf4j
@Service
public class DaoEditInformationNftOwnerSetChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[DaoEditInformationNftOwnerSetChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        // 查看数据库中目前绑定的信息
        NodePermissionNft nodePermissionNft = nodePermissionNftService.selectNodePermissionNft(projectId, NodePermissionTypeEnum.Edit_SubNode.getType());
        if (nodePermissionNft == null) {
            log.error("[DaoEditInformationNftOwnerSetChainService] nodePermissionNft is null, projectId:{},permissionType:{}", projectId, NodePermissionTypeEnum.Edit_SubNode.getType());
        ...
        if (nodePermissionNft.getTransactionHash().equalsIgnoreCase(transactionDto.getTransactionHash())){
            ...
            log.info("[DaoEditInformationNftOwnerSetChainService] nodePermissionNft is equal, projectId:{}", projectId);
            return;
        }
        ...
        if (dao == null) {
            log.error("[DaoEditInformationNftOwnerSetChainService] dao is null, erc721Address:{}", erc721Address);
        ...
        if (work == null || !Objects.equals(work.getWorkStatus(), WorkStatusEnum.CASTED.getStatus())) {
            log.error("[DaoEditInformationNftOwnerSetChainService] work is null, dao id:{}, tokenId:{}", dao.getId(), tokenId);
