...
/**
 * 转移Edit Strategies权限(所有dao均有)
 *
 * @description: 转移Edit Strategies权限(所有dao均有)
 * @author: zhyyao
 * @create: 2024-07-17 13:43
 **/
@Slf4j
@Service
public class DaoEditStrategyNftOwnerSetChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[DaoEditStrategyNftOwnerSetChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        // 查看数据库中目前绑定的信息
        NodePermissionNft nodePermissionNft = nodePermissionNftService.selectNodePermissionNft(projectId, NodePermissionTypeEnum.Edit_Strategies.getType());
        if (nodePermissionNft == null) {
            log.error("[DaoEditStrategyNftOwnerSetChainService] nodePermissionNft is null, projectId:{},permissionType:{}", projectId, NodePermissionTypeEnum.Edit_Strategies.getType());
        ...
        if (nodePermissionNft.getTransactionHash().equalsIgnoreCase(transactionDto.getTransactionHash())){
            ...
            log.info("[DaoEditStrategyNftOwnerSetChainService] nodePermissionNft is equal, projectId:{}", projectId);
            return;
        }
        ...
        if (dao == null) {
            log.error("[DaoEditStrategyNftOwnerSetChainService] dao is null, erc721Address:{}", erc721Address);
        ...
        if (work == null || !Objects.equals(work.getWorkStatus(), WorkStatusEnum.CASTED.getStatus())) {
            log.error("[DaoEditStrategyNftOwnerSetChainService] work is null, dao id:{}, tokenId:{}", dao.getId(), tokenId);
