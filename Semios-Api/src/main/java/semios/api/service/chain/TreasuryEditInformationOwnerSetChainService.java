...
/**
 * 转移编辑seed nodes information的权限(仅main dao)
 *
 * @description: 转移编辑seed nodes information的权限(仅main dao)
 * @author: zhyyao
 * @create: 2024-07-17 13:43
 **/
@Slf4j
@Service
public class TreasuryEditInformationOwnerSetChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[TreasuryEditInformationOwnerSetChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        // 查看数据库中目前绑定的信息
        NodePermissionNft nodePermissionNft = nodePermissionNftService.selectNodePermissionNft(projectId, NodePermissionTypeEnum.Edit_SeedNode.getType());
        if (nodePermissionNft == null) {
            log.error("[TreasuryEditInformationOwnerSetChainService] nodePermissionNft is null, projectId:{},permissionType:{}", projectId, NodePermissionTypeEnum.Edit_SeedNode.getType());
        ...
        if (nodePermissionNft.getTransactionHash().equalsIgnoreCase(transactionDto.getTransactionHash())){
            ...
            log.info("[TreasuryEditInformationOwnerSetChainService] nodePermissionNft is equal, projectId:{}", projectId);
            return;
        }
        ...
        if (dao == null) {
            log.error("[TreasuryEditInformationOwnerSetChainService] dao is null, erc721Address:{}", erc721Address);
        ...
        if (work == null || !Objects.equals(work.getWorkStatus(), WorkStatusEnum.CASTED.getStatus())) {
            log.error("[TreasuryEditInformationOwnerSetChainService] work is null, dao id:{}, tokenId:{}", dao.getId(), tokenId);
