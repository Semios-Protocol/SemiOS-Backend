...
/**
 * 转移设置top-up账户分流权限(仅main dao)
 *
 * @description: 转移设置top-up账户分流权限(仅main dao)
 * @author: zhyyao
 * @create: 2024-07-17 13:43
 **/
@Slf4j
@Service
public class TreasurySetTopUpRatioOwnerSetChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[TreasurySetTopUpRatioOwnerSetChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        // 查看数据库中目前绑定的信息
        NodePermissionNft nodePermissionNft = nodePermissionNftService.selectNodePermissionNft(projectId, NodePermissionTypeEnum.Topup_Ratio.getType());
        if (nodePermissionNft == null) {
            log.error("[TreasurySetTopUpRatioOwnerSetChainService] nodePermissionNft is null, projectId:{},permissionType:{}", projectId, NodePermissionTypeEnum.Topup_Ratio.getType());
        ...
        if (nodePermissionNft.getTransactionHash().equalsIgnoreCase(transactionDto.getTransactionHash())){
            ...
            log.info("[TreasurySetTopUpRatioOwnerSetChainService] nodePermissionNft is equal, projectId:{}", projectId);
            return;
        }
        ...
        if (dao == null) {
            log.error("[TreasurySetTopUpRatioOwnerSetChainService] dao is null, erc721Address:{}", erc721Address);
        ...
        if (work == null || !Objects.equals(work.getWorkStatus(), WorkStatusEnum.CASTED.getStatus())) {
            log.error("[TreasurySetTopUpRatioOwnerSetChainService] work is null, dao id:{}, tokenId:{}", dao.getId(), tokenId);
