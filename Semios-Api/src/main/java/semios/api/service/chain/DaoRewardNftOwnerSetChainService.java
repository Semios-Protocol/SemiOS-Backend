...
/**
 * 转移SubNodes Creator收益权限(所有dao均有)
 *
 * @description: 转移SubNodes Creator收益权限(所有dao均有)
 * @author: zhyyao
 * @create: 2024-07-17 13:43
 **/
@Slf4j
@Service
public class DaoRewardNftOwnerSetChainService implements SubscriberChainService {
    ...
    public void handleTrade(TransactionDto transactionDto) throws Exception {
        log.info("[DaoRewardNftOwnerSetChainService] transactionDao:{}", JacksonUtil.obj2json(transactionDto));
        ...
        // 查看数据库中目前绑定的信息
        NodePermissionNft nodePermissionNft = nodePermissionNftService.selectNodePermissionNft(projectId, NodePermissionTypeEnum.Proceeds_Ratio.getType());
        if (nodePermissionNft == null) {
            log.error("[DaoRewardNftOwnerSetChainService] nodePermissionNft is null, projectId:{},permissionType:{}", projectId, NodePermissionTypeEnum.Proceeds_Ratio.getType());
        ...
        if (nodePermissionNft.getTransactionHash().equalsIgnoreCase(transactionDto.getTransactionHash())){
            ...
            log.info("[DaoRewardNftOwnerSetChainService] nodePermissionNft is equal, projectId:{}", projectId);
            return;
        }
        ...
        if (dao == null) {
            log.error("[DaoRewardNftOwnerSetChainService] dao is null, erc721Address:{}", erc721Address);
        ...
        if (work == null || !Objects.equals(work.getWorkStatus(), WorkStatusEnum.CASTED.getStatus())) {
            log.error("[DaoRewardNftOwnerSetChainService] work is null, dao id:{}, tokenId:{}", dao.getId(), tokenId);
