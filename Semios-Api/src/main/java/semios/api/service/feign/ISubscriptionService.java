...
public interface ISubscriptionService {
    ...
    Result<String> ethGetTransactionFirstLog(@RequestParam(required = false) String netWork,
                                             @RequestParam(required = false) String transactionHash);

    @PostMapping("/event/query/transaction")
    Result<String> ethGetTransactionReceipt(@RequestParam(required = false) String netWork,
                                            @RequestParam(required = false) String transactionHash);
    ...
    @PostMapping(value = "/event/query/blockTime")
    Result<String> queryBlockTime(@RequestParam(required = false) String netWork, @RequestParam(required = false) String blockNumber);

    @PostMapping(value = "/event/eth/getBalance")
    Result<String> ethGetBalance(@RequestParam(required = false) String netWork,
                                 @RequestParam(required = false) String contractAddress);

    @PostMapping(value = "/event/query/transaction/hash")
    Result<String> ethGetTransactionByHash(@RequestParam(required = false) String netWork,
                                           @RequestParam(required = false) String transactionHash);
