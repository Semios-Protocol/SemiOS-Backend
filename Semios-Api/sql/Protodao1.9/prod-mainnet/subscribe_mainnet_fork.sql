# 数据全部清除，表结构从test中取
CREATE SCHEMA `subscribe_fork` DEFAULT CHARACTER SET utf8mb4;

create table if not exists subscribe_fork.block_height
(
...
create table if not exists subscribe_fork.sub_num_value
(
...
create table if not exists subscribe_fork.subscriber
(
...
create table if not exists subscribe_fork.transaction
(
    id                int auto_increment
        primary key,
